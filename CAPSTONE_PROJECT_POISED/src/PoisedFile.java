...
public class PoisedFile {

	public static Statement getDatabaseStatement() {
		try {
			Connection conn = 
				DriverManager.getConnection("jdbc:mysql://localhost:3306/PoisedPMS?useSSL=false&allowPublicKeyRetrieval=true",
						"root", "Dew@ld844");
	 ...
	 */
	public static ArrayList<PoisedProject> getProjects() {
		// Create ArrayList of projects
		ArrayList<PoisedProject> projects = new ArrayList<PoisedProject>();
		...
		try {
			...
			while(results.next()) {
				...
				PoisedPerson architect = new PoisedPerson("Architect", results.getString("architect.NAME"), results.getString("architect.PHONE_NUM"),
						results.getString("architect.EMAIL"),results.getString("architect.ADDRESS"));
				project.architect = architect;
				
				PoisedPerson customer = new PoisedPerson("Customer", results.getString("customer.NAME"), results.getString("customer.PHONE_NUM"),
						results.getString("customer.EMAIL"),results.getString("customer.ADDRESS"));
				project.customer = customer;
				
				PoisedPerson contractor = new PoisedPerson("Contractor", results.getString("contractor.NAME"), results.getString("contractor.PHONE_NUM"),
	...
	public static void addProjectsToDatabase(PoisedProject project) {
	...
	public static void updateDatabase(String field, int project) {
		...
		String updateDeadline = "update project set DEADLINE = '" + PoisedMenu.projects.get(project - 1 ).deadline + "' where CUST_ID = " + project;
		String updateAmountPaid = "update project set AMOUNT_PAID = " + PoisedMenu.projects.get(project - 1).totalPaidDate + " where CUST_ID = " + project;
		String updateContractor  = "update contractor set PHONE_NUM = '" + PoisedMenu.projects.get(project -1).contractor.phoneNum + "', EMAIL = '" +
				PoisedMenu.projects.get(project - 1).contractor.email + "' where ID = " + project;
		String updateFinalised = "update project set FINALISED = '" + PoisedMenu.projects.get(project - 1).finalised + "' where CUST_ID = " + project;
